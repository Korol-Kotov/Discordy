...
import me.korolkotov.discordy.annotations.SubCommand
import me.korolkotov.discordy.config.ConfigManager
import me.korolkotov.discordy.logger.Logger
import me.korolkotov.discordy.util.MessageService
...
abstract class CommandExecutor : TabExecutor {
    ...
    private fun runCommand(sender: CommandSender, wrapper: CommandWrapper, args: Array<out String>): CommandResult {
        ...
        runCatching {
            if (command.permissionNode.isNotEmpty() &&
                !sender.hasPermission("discordy.${command.permissionNode}")
    ...
    fun sendHelpMessage(sender: CommandSender) {
        ...
        commandMethods.values.forEach { wrappers ->
            wrappers.filter { sender.hasPermission("discordy.${it.getCommand().permissionNode}") }
        ...
        runCatching {
            if (args.size == 1) {
                for (cmd in commandMethods.keys) {
                    ...
                    if (command.permissionNode.isNotEmpty() && !sender.hasPermission("discordy.${command.permissionNode}"))
                        continue
